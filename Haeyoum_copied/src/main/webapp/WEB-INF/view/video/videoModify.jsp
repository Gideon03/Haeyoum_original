...
<div class="blog-holder center-relative">                   
	...
	<form action="${pageContext.request.contextPath}/group/video/videoModify" method="post" enctype="multipart/form-data">
		<input type = "hidden" value="${videoView.con_id}" name = "con_id" >
			<div>
        		제목: <input required="required" name="video_name" size="80" value="${videoView.video_name}"/>
    		</div>
    
    		<div>
      			변경할 파일  <input type="file" name="flie" required="required"/>
    		</div>
    		
    		<div>
       			 내용: <input name="video_text" size="300" value="${videoView.video_text}" />
    		</div>
...
<script>
	...
	var mapContainer = document.getElementById('map'), // 지도를 표시할 div 
    	mapOption = { 
        	center: new daum.maps.LatLng(${videoView.map_latitude}, ${videoView.map_longitude}), // 지도의 중심좌표
        	level: 3 // 지도의 확대 레벨
    	};
...
$("#submit").click(function() {
	if(document.getElementById('pointLat').value===""){
		document.getElementById('pointLat').value = "${videoView.map_latitude}";
		document.getElementById('pointLng').value = "${videoView.map_longitude}";
	    }
